...
use GD;
#use GD::SVG;###comment above and uncomment this
...
#    }
    ...
    Draw();
    RealAttachText();
    open( OUT, ">$outdir/$total_count.$type" ) || die $!;
    if($png)
    {
    	print OUT $im->png;
    }
...
#$im->arc(0.5*($begin_pixel+$end_pixel),$y_pixel-0.5*$exon_height,
#    $end_pixel-$begin_pixel+1,$exon_height,0,360,$element_color{$id});
#$im->fill(0.5*($begin_pixel+$end_pixel),$y_pixel-0.5*$exon_height,$element_color{$id});
    }
    else
    {
...
sub InitPic
{
    ...
    {
    	$im = new GD::SVG::Image( $Width + 1, $Height + 1 );
    }
